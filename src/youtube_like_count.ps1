...
$countValue = $realtimeData.videoActions.menuRenderer.topLevelButtons[0].segmentedLikeDislikeButtonViewModel.likeButtonViewModel.likeButtonViewModel.toggleButtonViewModel.toggleButtonViewModel.toggledButtonViewModel.buttonViewModel.title;
#if ($true) {
#    # 自分が高評価していると YouTube から取得できるデータとズレる
#    # そこを補正するのかどうか……
#}

$htmlTemplateSource.Replace("%like_count_value%", $countValue) | Set-Content -Path $htmlFilePath -Encoding UTF8
